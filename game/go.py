...
class Board(object):
    ...
    def put_stone(self, point, check_legal=False):
        ...
        return True
    def to_dict(self):
        """Convert the board's state into a dictionary."""
        return {
            'winner': self.winner,
            'next': self.next,
            'legal_actions': self.legal_actions,
            'end_by_no_legal_actions': self.end_by_no_legal_actions,
            'counter_move': self.counter_move,
            'groups': {
                'BLACK': [str(group) for group in self.groups['BLACK']],
                'WHITE': [str(group) for group in self.groups['WHITE']]
            },
            'endangered_groups': [str(group) for group in self.endangered_groups],
            'removed_groups': [str(group) for group in self.removed_groups],
        }
